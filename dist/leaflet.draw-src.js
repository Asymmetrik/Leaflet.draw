...
L.HexLayer = L.Class.extend({
	...
	_initContainer : function() {
		...
		if (!this._container || overlayPane.empty) {
			...
			this._container = d3.select(overlayPane).append('svg').attr(
					'class', 'leaflet-layer leaflet-zoom-hide');
		}
	},
	...
	_update : function() {
		...
		this._container.attr('width', width).attr('height', height).style(
				'margin-left', marginLeft + 'px').style('margin-top',
				marginTop + 'px');
		...
		if (!(zoom in this._levels)) {
			...
			this._levels[zoom].attr('transform', 'translate(' + -marginLeft + ',' + -marginTop + ')');
		}
		this._setLevel(zoom);
	},
	...
	_createHexagons : function(container) {
		var layout = this._layout, data = this._data.features.map(function(d) {
			return this._project(d.geometry.coordinates);
		}, this), bins = layout(data), hexagons = container.selectAll(
				'.hexagon').data(bins);
	...
	_translateBounds : function(d3Bounds) {
		var nw = this._project([ d3Bounds[0][0], d3Bounds[1][1] ]), se = this
				._project([ d3Bounds[1][0], d3Bounds[0][1] ]);
